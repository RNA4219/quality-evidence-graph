...
`ipo_controlled` は、上場準備レベルの変更管理、品質判定、例外承認、証跡保全を説明するための Gate profile である。MVP では実装準備契約を固定し、release Go は adapter / evaluator / record CLI 実装後に別途判定する。
...
IPO controlled release Gate は、実装と fixture 証跡が揃うまで `no_go` とする。
