...
R8〜R13の[共通matrix](../project/cli-boundary-fixes-2026-09-11.md)は、cwd（repo root/子階層/target自身）、明示/自動target、baselineの資格/対象、diffの評価可否、source/tarball/Action/初期化runtimeを組み合わせる。単一cwdの成功を他の導入形態へ転用しない。誤成功4件はすべて拒否し、unverifiedを解消と表示しないことを必須oracleとする。
